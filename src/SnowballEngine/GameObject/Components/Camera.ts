...
export class Camera extends Component<CameraEventTypes> {
    ...
    public get size(): Vector2 {
        ...
        const pxSize = this._screenSize.clone
            .scale(Client.resolution)
            .scale(0.01)
            .setLength(this._size.magnitude);
    ...
    public set screenSize(val: Vector2) {
        ...
        if (val.x !== this._screenSize.x || val.y !== this._screenSize.y)
            Debug.warn(`Camera(${this.gameObject.name}).screenSize was clamped to 0.0001-100`);
    }
    ...
    public render(pixi: PIXI): void {
        ...
        pixi.container.scale.copyFrom(
            new Vector2(
                pixi.canvas.width / this.size.x / (100 / this._screenSize.x),
                pixi.canvas.height / this.size.y / (100 / this._screenSize.y)
            ).scale(globalTransform.scale)
        );
        pixi.container.position.copyFrom(
            this.worldToCameraPoint(globalTransform.position.scale(-1)).add(
                new Vector2(
                    pixi.canvas.width / (100 / this.screenPosition.x),
                    pixi.canvas.height / (100 / this.screenPosition.y)
                )
            )
        );
        ...
        (<Graphics>pixi.container.mask)
            ...
            .drawRect(
                pixi.canvas.width / (100 / this.screenPosition.x),
                pixi.canvas.height / (100 / this.screenPosition.y),
                pixi.canvas.width / (100 / this._screenSize.x),
                pixi.canvas.height / (100 / this._screenSize.y)
            )
            .endFill();
    ...
    public worldToCamera(vec: Vector2): Vector2 {
        return new Vector2(
            (vec.x * this.gameObject.scene.cameraManager.canvas.width) /
                this.size.x /
                (100 / this._screenSize.x),
            (vec.y * this.gameObject.scene.cameraManager.canvas.height) /
                this.size.y /
                (100 / this._screenSize.y)
        );
    }
