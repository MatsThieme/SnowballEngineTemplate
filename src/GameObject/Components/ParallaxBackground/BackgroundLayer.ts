...
export class BackgroundLayer implements Disposable {
    ...
    public constructor(
        speed: number,
        backgroundLayerAsset: BackgroundLayerAsset,
        container: Container,
    ...
    private toSprite(asset: BackgroundLayerSprite): BackgroundLayerSprite {
        ...
        const sprite = (asset.sprite = new TilingSprite(
            asset.asset.getPIXITexture()!,
            asset.size.x * 2,
            asset.size.y - pxScale
        ));
    ...
    public updateSpriteForCamera(camera: Camera): void {
        const spritePos = Vector2.sub(
            Transform.toLocal(
                camera.gameObject.transform,
                Transform.fromPIXI(this._container, this._parallaxBackground.gameObject.transform)
            ).position,
            this.backgroundLayerSprite.spriteCenter
        ).scale(new Vector2(this.speed, -this.speed));

        this.backgroundLayerSprite.sprite.position.copyFrom(
            spritePos.add(new Vector2(this.backgroundLayerSprite.size.x / 2))
        );
    }
